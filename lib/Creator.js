...
module.exports = class Creator extends EventEmitter {
  ...
  async create(cliOptions = {}, preset = null) {
    ...
    await clearConsole()
    log(chalk.blue.bold(`@jibenziliao/create-vite-react-ts CLI v${require('../package.json').version}`))
    ...
    const { pkgName, pkgVers, pkgDes } = await inquirer.prompt([
      ...
      },
      {
        name: 'pkgDes',
        message: `请输入项目简介`,
        default: 'project created by @jibenziliao/create-vite-react-ts'
      }
    ])
    ...
    if (gitCommitFailed) {
      warn(`因您的git username或email配置不正确，无法为您初始化git commit，\n` + `请稍后自行git commit。\n`)
    }
  }
